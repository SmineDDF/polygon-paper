...
type DrawPolygon = (points: paper.Point[]) => void;
...
class DrawingTool implements IAttachedTool {
    ...
    private onMouseDownHandler = (event: paper.ToolEvent) => {
        ...
        if (this.isDrawing) {
            const isConnectingToStartingPoint =
                areSamePoint(this.drawingPoints[0], event.point, GlobalConfig.get('pointerEventsTolerance'));
        ...
        if (hitTester.didHitPolygon || hitTester.didHitPolygonGroup) {
            return;
        }
    ...
    private onMouseMoveHandler = (event: paper.ToolEvent) => {
        ...
        const isHoveringFirstPathPoint = areSamePoint(
            event.point,
            this.drawnPath.firstSegment.point,
            GlobalConfig.get('pointerEventsTolerance')
        );
